...
import { Signature } from "../types/sms.js";

//6자리 random 숫자 생성
export const create6DigitCode = ():string=> {
    const code = Math.floor(100000 + Math.random() * 900000);
    return code + "";
}

// redis에 key-value 형태로 저장 : 3분후에 exprire
export const saveAuthCode = (key:string, value:string):void => {
    redisClient.set(key, value);
};

export const sendMessage = async (to:string, code:string):Promise<boolean> => {
...
export const compareAuthCode = async (key:string, code:string):Promise<boolean>=> {
